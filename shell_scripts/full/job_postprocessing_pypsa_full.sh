#!/usr/bin/env bash
#BSUB -q man
#BSUB -J pypsa_postprocess_full
#BSUB -n 8
#BSUB -R "span[hosts=1]"
#BSUB -R "rusage[mem=8GB]"
#BSUB -M 8GB
#BSUB -W 08:00
#BSUB -o hpc_output_and_error_files/Output_Postprocessing_full_%J.out
#BSUB -e hpc_output_and_error_files/Output_Postprocessing_full_%J.err
...
NETWORK_PATH="${2:-$PYPSA_EUR_DIR/results/kupferzell_2024_full/networks/base_s_256_elec_.nc}"
...
if [[ -z "$POWERPLANTS_CSV" ]]; then
  if [[ -n "$RUN_FOLDER" && -f "$PYPSA_EUR_DIR/resources/$RUN_FOLDER/powerplants_s_256.csv" ]]; then
    ...
    POWERPLANTS_CSV="$PYPSA_EUR_DIR/resources/kupferzell_2024_full/powerplants_s_256.csv"
  fi
fi
